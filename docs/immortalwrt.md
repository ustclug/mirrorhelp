...
=== "25.12.0 及以后"
    ...
    SSH 登录路由器编辑 `/etc/apk/repositories.d/distfeeds.list` 文件，将源地址 `https://downloads.immortalwrt.org` 或 `https://mirrors.vsean.net/openwrt` 更改为 `https://mirrors.ustc.edu.cn/immortalwrt`。
    ...
    sed -e 's|https://downloads.immortalwrt.org|https://mirrors.ustc.edu.cn/immortalwrt|g' \
        ...
        -i.old /etc/apk/repositories.d/distfeeds.list
    ```
...
!!! tip
    ...
    === "25.12.0 及以后"
        ...
        ```shell
        uci set system.@imm_init[0].apk_mirror='https://mirrors.ustc.edu.cn/immortalwrt'
        uci commit system
        ```
